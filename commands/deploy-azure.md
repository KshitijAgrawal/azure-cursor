...
1. **Check restricted mode**: If `azure-cursor.config.json` or `.cursor/azure-cursor.json` exists with `restrictedMode: true`, enforce scope (readOnly or resourceGroup). See azure-restricted-mode rule. Do not deploy if scope is readOnly.
...
6. **Verify**: Confirm deployment succeeded and endpoints are reachable.
